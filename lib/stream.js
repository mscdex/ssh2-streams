...
SSH2Stream.prototype.x11 = function(chan, initWindow, maxPacket, addr, port) {
  if (!this.server)
    throw new Error('Server-only method called in client mode');

  var addrLen = Buffer.byteLength(addr),
      p = 24 + addrLen,
  ...
  buf.write(addr, 24, addrLen, 'ascii');

  buf.writeUInt32BE(port, p, true);
