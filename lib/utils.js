...
module.exports = {
  ...
  genPublicKey: function(privKeyInfo) {
    ...
    if (privKey[0] === 0x30) {
      ...
      if (privKeyInfo.type === 'rsa') {
        var e = privKey.slice(eStart, eStart + eLen),
            n = privKey.slice(nStart, nStart + nLen);
        asnWriter.startSequence();
        ...
        var p = privKey.slice(pStart, pStart + pLen),
            q = privKey.slice(qStart, qStart + qLen),
            g = privKey.slice(gStart, gStart + gLen),
            y = privKey.slice(yStart, yStart + yLen);
