...
function DSASigBERToBare(signature) {
  ...
  r.copy(newSig, 0, r.length - 20);
  s.copy(newSig, 20, s.length - 20);
  return newSig;
}
...
function DSASigBareToBER(signature) {
  ...
  asnWriter.writeBuffer(rBuf, Ber.Integer);
  asnWriter.writeBuffer(sBuf, Ber.Integer);
...
function decryptKey(keyInfo, passphrase) {
  ...
  if (keyInfo.ppk) {
    ...
    key = Buffer.concat([
      crypto.createHash('sha1')
            .update('\x00\x00\x00\x00' + passphrase)
            .digest(),
      crypto.createHash('sha1')
            .update('\x00\x00\x00\x01' + passphrase)
            .digest()
    ]);
    ...
    key = crypto.createHash('md5')
                .update(passphrase + iv.slice(0, 8))
                .digest();
  ...
  out = dc.update(keyInfo.private);
  out = Buffer.concat([out, dc.final()]);
  ...
  if (keyInfo.privateOrig) {
    ...
    var newOrig = /^(.+(?:\r\n|\n))/.exec(orig)[1];
    var b64key = out.toString('base64');
