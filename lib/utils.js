...
var RE_STREAM = /^arcfour/i,
    RE_GCM = /^aes\d+-gcm/i,
    RE_KEY_LEN = /(.{64})/g,
    MAX_STRING_LEN = 2400, // taken from dropbear
...
function readString(buffer, start, encoding, stream, cb) {
  ...
  if (len > MAX_STRING_LEN || left < (4 + len)) {
    stream && stream._cleanup(cb);
    return false;
  }
