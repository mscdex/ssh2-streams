// TODO: support EXTENDED request/response packets
...
function SFTPStream(cfg, remoteIdentRaw) {
  if (!(this instanceof SFTPStream))
    return new SFTPStream(cfg);
  ...
  this._state = {
    ...
    // client
    maxDataLen: (remoteIdentRaw && RE_OPENSSH.test(remoteIdentRaw)
                 ? OPENSSH_MAX_DATA_LEN
                 : 32768),
    requests: {}
  };
...
SFTPStream.prototype._transform = function(chunk, encoding, callback) {
  ...
  while (true) {
      ...
      if (!buffer) {
        ...
        // here we read the right-most 5 bytes from pktHdrBuf/buffer
      ...
      if (pktType === RESPONSE.VERSION || pktType === REQUEST.INIT) {
        /*
          ...
          <extension data>
        */
        version = state.version = buffer.readUInt32BE(0, true);
        if (version !== 3) {
        ...
        /*
          ...
          uint32     id
        */
        var id = buffer.readUInt32BE(0, true);
        if (!server) {
          var req = requests[id],
              cb = req && req.cb;
          ...
          if (req && cb) {
            if (pktType === RESPONSE.STATUS) {
              /*
                ...
                string     language tag
              */
              var code = buffer.readUInt32BE(4, true),
                  msg = readString(buffer, 8, 'utf8'),
                  lang = readString(buffer, buffer._pos, 'ascii');
              ...
              /*
                string     handle
              */
              cb(undefined, readString(buffer, 4));
              ...
              */
              if (req.buffer) {
                ...
                var dataLen = buffer.readUInt32BE(4, true),
                    reqBufLen = req.buffer.length;
                if (dataLen > reqBufLen) {
                  // truncate response data to fit expected size
                  buffer.writeUInt32BE(reqBufLen, 4, true);
                }
                cb(undefined, readString(buffer, 4, req.buffer), dataLen);
              } else
                cb(undefined, readString(buffer, 4));
              ...
              /*
                ...
                repeats count times:
                        ...
                        ATTRS      attrs
              */
              var namesLen = buffer.readUInt32BE(4, true),
                  names = [],
                  filename,
                  longname,
                  attrs;
              ...
              for (var i = 0; i < namesLen; ++i) {
                ...
                // filenames
                filename = readString(buffer, buffer._pos, 'utf8');
                // `longname` only exists in SFTPv3 and since it typically will
                // contain the filename, we assume it is also UTF-8
                longname = readString(buffer, buffer._pos, 'utf8');
                attrs = readAttrs(buffer, buffer._pos);
                names.push({
                  filename: filename,
                  longname: longname,
                  attrs: attrs
              ...
              /*
                ATTRS      attrs
              */
              cb(undefined, readAttrs(buffer, 4));
            }
          }
        } else {
          // server
          ...
          if (listenerCount(this, evName)) {
            if (pktType === REQUEST.OPEN) {
              /*
                ...
                ATTRS         attrs
              */
              var filename = readString(buffer, 4, 'utf8'),
                  pflags = buffer.readUInt32BE(buffer._pos, true),
                  attrs = readAttrs(buffer, buffer._pos + 4);
              ...
              /*
                string     handle
              */
              this.emit(evName, id, readString(buffer, 4));
              ...
              /*
                ...
                uint32     len
              */
              var handle = readString(buffer, 4),
                  offset = readUInt64BE(buffer, buffer._pos),
                  len = buffer.readUInt32BE(buffer._pos, true);
              ...
              */
              var handle = readString(buffer, 4),
                  offset = readUInt64BE(buffer, buffer._pos),
                  data = readString(buffer, buffer._pos);
              ...
              /*
                string     path
              */
              this.emit(evName, id, readString(buffer, 4, 'utf8'));
              ...
              */
              this.emit(evName,
                        id,
                        readString(buffer, 4, 'utf8'),
                        readAttrs(buffer, buffer._pos));
              ...
              */
              this.emit(evName,
                        id,
                        readString(buffer, 4, 'utf8'),
                        readString(buffer, buffer._pos, 'utf8'));
            }
          } else {
      ...
      if (server && buffer[4] !== REQUEST.INIT) {
        var errCode = (buffer[4] === REQUEST.EXTENDED
                       ...
                       : STATUS_CODE.FAILURE);
        this.status(buffer.readUInt32BE(5, true), errCode);
      }
...
function readAttrs(buf, p) {
  ...
  */
  ...
  if (flags & ATTR.SIZE) {
    attrs.size = readUInt64BE(buf, p);
    p += 8;
  }
  if (flags & ATTR.UIDGID) {
    attrs.uid = buf.readUInt32BE(p, true);
    p += 4;
    attrs.gid = buf.readUInt32BE(p, true);
    p += 4;
  }
  if (flags & ATTR.PERMISSIONS) {
    // backwards compatibility
    attrs.permissions = attrs.mode = buf.readUInt32BE(p, true);
    p += 4;
  }
  if (flags & ATTR.ACMODTIME) {
    attrs.atime = buf.readUInt32BE(p, true);
    p += 4;
    attrs.mtime = buf.readUInt32BE(p, true);
    p += 4;
  }
  if (flags & ATTR.EXTENDED) {
    // TODO: read/parse extended data
    var extcount = buf.readUInt32BE(p, true);
    p += 4;
    for (var i = 0; i < extcount; ++i)
      p += 4 + buf.readUInt32BE(p, true);
  }
