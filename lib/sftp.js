      ...
      if (pktType === RESPONSE.VERSION || pktType === REQUEST.INIT) {
        ...
        */
        ...
        /*
          ...
          uint32     id
        */
        var id = readInt(buffer, 0, this, callback);
        if (id === false)
          return;
        if (!server) {
          var req = requests[id],
            ...
            if (pktType === RESPONSE.STATUS) {
              ...
              */
              ...
              /*
                string     handle
              */
              var handle = readString(buffer, 4, this, callback);
              if (handle === false)
                return;
              cb(undefined, handle);
              ...
              */
              ...
              /*
                ATTRS      attrs
              */
              var attrs = readAttrs(buffer, 4, this, callback);
              if (attrs === false)
                return;
              cb(undefined, attrs);
            }
          ...
          if (listenerCount(this, evName)) {
            if (pktType === REQUEST.OPEN) {
              /*
                ...
                ATTRS         attrs
              */
              var filename = readString(buffer, 4, 'utf8', this, callback);
              if (filename === false)
                return;
              var pflags = readInt(buffer, buffer._pos, this, callback);
              if (pflags === false)
                return;
              var attrs = readAttrs(buffer, buffer._pos + 4, this, callback);
              if (attrs === false)
                return;
              ...
              */
              ...
              if (handle === false)
                return;
              var offset = readUInt64BE(buffer, buffer._pos, this, callback);
              if (offset === false)
                return;
              ...
              */
              ...
              if (offset === false)
                return;
              var data = readString(buffer, buffer._pos, this, callback);
              if (data === false)
                return;
              ...
              /*
                string     path
              */
              var path = readString(buffer, 4, 'utf8', this, callback);
              if (path === false)
                return;
...
function readUInt64BE(buffer, p, stream, cb) {
