...
var consts = require('./constants'),
    ...
    readString = require('./utils').readString,
    readInt = require('./utils').readInt;
...
var EMPTY_BUFFER = new Buffer(0),
    ...
    NO_TERMINAL_MODES_BUFFER = new Buffer([TERMINAL_MODE.TTY_OP_END]),
...
function parsePacket(self, callback) {
  ...
  if (type === MESSAGE.CHANNEL_DATA) {
    /*
      ...
      string    data
    */
    var chan = readInt(payload, 1, self, callback);
    if (chan === false)
      return false;
    var data = readString(payload, 5, self, callback);
    if (data === false)
      return false;
    ...
    */
    ...
    if (dataType === false)
      return false;
    var data = readString(payload, 9, self, callback);
    if (data === false)
      return false;
    ...
    */
    ...
    if (chanType === false)
      return false;
    var sender = readInt(payload, payload._pos, self, callback);
    if (sender === false)
      return false;
    var window = readInt(payload, payload._pos += 4, self, callback)
    if (window === false)
      return false;
    var packetSize = readInt(payload, payload._pos += 4, self, callback);
    if (packetSize === false)
      return false;
    var channel;
    ...
    if (chanType === 'forwarded-tcpip') {
      ...
      */
      var destIP = readString(payload, payload._pos += 4, 'ascii', self, callback);
      if (destIP === false)
        return false;
      var destPort = readInt(payload, payload._pos, self, callback);
      if (destPort === false)
        return false;
      var srcIP = readString(payload, payload._pos += 4, 'ascii', self, callback);
      if (srcIP === false)
        return false;
      var srcPort = readInt(payload, payload._pos, self, callback);
      if (srcPort === false)
        return false;
      channel = {
        type: chanType,
    ...
    */
    ...
    if (recipient === false)
      return false;
    var sender = readInt(payload, 5, self, callback);
    if (sender === false)
      return false;
    var window = readInt(payload, 9, self, callback);
    if (window === false)
      return false;
    var packetSize = readInt(payload, 13, self, callback);
    if (packetSize === false)
      return false;
    ...
    */
    ...
    if (reasonCode === false)
      return false;
    var description = readString(payload, payload._pos, 'utf8', self, callback);
    ...
    */
    ...
    var description = readString(payload, 5, 'utf8', self, callback);
    if (description === false)
      return false;
    ...
    */
    ...
    if (method === 'password') {
      ...
      if (pkSigned === undefined)
        return false;
      pkSigned = (pkSigned !== 0);
      var keyAlgo = readString(payload, payload._pos, 'ascii', self, callback);
      if (keyAlgo === false)
        return false;
      var key = readString(payload, payload._pos, self, callback);
      if (key === false)
        return false;
      var signature,
          blob;
      ...
      if (pkSigned) {
        signature = readString(payload, payload._pos, self, callback);
        if (signature === false)
          return false;
      ...
      if (skipLen === false)
        return false;
      methodData = readString(payload, payload._pos + 4 + skipLen, 'utf8', self, callback);
      if (methodData === false)
        return false;
    } else if (method !== 'none')
  ...
  if (request === 'exit-status') { // server->client
    ...
    */
    ...
    /*
      ...
      string    language tag
    */
    var signal = readString(payload, ++payload._pos, 'ascii', self, callback);
    if (signal === false)
      return false;
    ...
    */
    ...
    if (term === false)
      return false;
    var cols = readInt(payload, payload._pos, self, callback);
    if (cols === false)
      return false;
    var rows = readInt(payload, payload._pos += 4, self, callback);
    if (rows === false)
      return false;
    var width = readInt(payload, payload._pos += 4, self, callback);
    if (width === false)
      return false;
    var height = readInt(payload, payload._pos += 4, self, callback);
    if (height === false)
      return false;
    ...
    */
    ...
    if (protocol === false)
      return false;
    var cookie = readString(payload, payload._pos, self, callback);
    if (cookie === false)
      return false;
