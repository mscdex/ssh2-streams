...
SSH2Stream.prototype._transform = function(chunk, encoding, callback) {
  ...
  while (true) {
    ...
    if (instate.status === IN_INIT) {
      ...
      // (to accommodate for packet length field and MAC) and re-use that instead
      if (instate.pktExtra) {
...
SSH2Stream.prototype.forwardedTcpip = function(chan, initWindow, maxPacket, cfg) {
  ...
  var boundAddrLen = Buffer.byteLength(cfg.boundAddr),
      remoteAddrLen = Buffer.byteLength(cfg.remoteAddr),
      p = 36 + boundAddrLen,
      buf = new Buffer(1 + 4 + 15 + 4 + 4 + 4 + 4 + boundAddrLen + 4 + 4
                       + remoteAddrLen + 4);
