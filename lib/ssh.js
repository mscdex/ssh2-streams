...
SSH2Stream.prototype.authPK = function(username, pubKey, cbSign) {
  ...
  cbSign(buf, function(signature) {
    signature = convertSignature(signature, pubKeyType);
    if (signature === false)
    ...
    writeUInt32BE(sigbuf, 4 + pubKeyTypeLen + 4 + sigLen, p += pubKeyLen);
    writeUInt32BE(sigbuf, pubKeyTypeLen, p += 4);
    sigbuf.write(pubKeyType, p += 4, pubKeyTypeLen, 'ascii');
    writeUInt32BE(sigbuf, sigLen, p += pubKeyTypeLen);
