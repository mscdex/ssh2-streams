...
function check_KEXINIT(self, init) {
  ...
  if (i === len) {
    ...
    self.disconnect(DISCONNECT_REASON.KEY_EXCHANGE_FAILED);
    self.emit('error',
              new Error('Handshake failed: no matching client->server cipher'));
    return false;
  }
  ...
  if (i === len) {
    ...
    self.disconnect(DISCONNECT_REASON.KEY_EXCHANGE_FAILED);
    self.emit('error',
              new Error('Handshake failed: no matching server->client cipher'));
    return false;
  }
  ...
  if (i === len) {
    ...
    self.disconnect(DISCONNECT_REASON.KEY_EXCHANGE_FAILED);
    self.emit('error',
              new Error('Handshake failed: no matching client->server HMAC'));
    return false;
  }
  ...
  if (i === len) {
    ...
    self.disconnect(DISCONNECT_REASON.KEY_EXCHANGE_FAILED);
    self.emit('error',
              new Error('Handshake failed: no matching server->client HMAC'));
    return false;
  }
  ...
  if (i === len) {
    ...
    self.disconnect(DISCONNECT_REASON.KEY_EXCHANGE_FAILED);
    self.emit('error',
              new Error('Handshake failed: no matching client->server '
  ...
  if (i === len) {
    ...
    self.disconnect(DISCONNECT_REASON.KEY_EXCHANGE_FAILED);
    self.emit('error',
              new Error('Handshake failed: no matching server->client '
                        + 'compression algorithm'));
    return false;
  }
...
function onKEXDH_REPLY(self, info, verifiedHost) { // Client
  ...
  if (verifiedHost === undefined) {
    ...
    if (info.hostkey_format !== state.hostkeyFormat
      ...
      self.reset();
      self.emit('error',
                new Error('Handshake failed: host key format mismatch'));
      return false;
    }
    ...
    if (info.sig_format !== sig_format) {
      ...
      self.reset();
      self.emit('error',
                new Error('Handshake failed: signature format mismatch'));
      return false;
    }
  }
  ...
  switch (info.sig_format) {
    ...
    default:
      ...
      self.reset();
      self.emit('error',
                new Error('Handshake failed: signature format unsupported: '
                          + info.sig_format));
      return false;
  }
  ...
  if (!verified) {
    ...
    self.reset();
    self.emit('error',
              new Error('Handshake failed: signature verification failed'));
    return false;
  }
...
function parsePacket(self, callback) {
  ...
  if (outstate.status !== OUT_READY
    ...
    self.emit('error', new Error('Received unexpected packet'));
    return false;
  }
...
function parse_KEX(self, type, callback) {
  ...
  if (RE_GEX.test(state.kexdh)) {
    ...
    if (self.server) {
      ...
      self.disconnect(DISCONNECT_REASON.PROTOCOL_ERROR);
      self.emit('error', new Error('DH group exchange not supported by server'));
      return false;
    } else {
...
function parse_USERAUTH(self, type, callback) {
  ...
  if (authMethod === 'password') {
    ...
    self.disconnect(DISCONNECT_REASON.PROTOCOL_ERROR);
    self.emit('error',
              new Error('Invalid authentication method: ' + authMethod));
  }
}
