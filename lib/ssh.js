  ...
  while (true) {
    ...
    if (instate.status === IN_INIT) {
      ...
      if (outstate.status === OUT_READY
        ...
        instate.rekeyQueue[instate.rekeyQueue.length] = [seqno,instate.payload];
      }
...
function parsePacket(self, callback) {
  ...
  if (type === MESSAGE.CHANNEL_DATA) {
    ...
    */
    ...
    if (chan === false)
      return false;
    // TODO: MAX_CHAN_DATA_LEN here should really be dependent upon the channel's
    //       packet size. The ssh2 module uses 32KB, so we'll hard code this for
    //       now ...
