...
function SSH2Stream(cfg) {
  ...
  this.on('header', function(header) {
    if (header.versions.protocol !== '1.99'
      ...
      self.removeAllListeners('header');
...
function KEXINIT(self) { // client/server
  ...
  */
  ...
  // TODO: remove KEX method modification when we support group exchange on
  // server
  var kex_list_size = ALGORITHMS.KEX_LIST_SIZE,
      kex_list = ALGORITHMS.KEX_LIST;
  if (self.server && ~kex_list.toString().indexOf('group-exchange')) {
    kex_list = ALGORITHMS.KEX_LIST.toString().split(',');
