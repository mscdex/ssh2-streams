...
var consts = require('./constants'),
    ...
    parseKey = require('./utils').parseKey,
    decryptKey = require('./utils').decryptKey,
    genPublicKey = require('./utils').genPublicKey;
...
var MESSAGE = consts.MESSAGE,
    ...
    SIGNALS = consts.SIGNALS,
    MODULE_VER = require('../package.json').version,
    I = 0,
    IN_INIT = I++,
    IN_GREETING = I++,
    IN_HEADER = I++,
    ...
    IN_PACKET = I++,
    IN_PACKETDATA = I++,
    IN_PACKETDATAVERIFY = I++,
    IN_PACKETDATAAFTER = I++,
    OUT_INIT = I++,
    OUT_READY = I++,
    OUT_REKEYING = I++,
...
var EMPTY_BUFFER = new Buffer(0),
    ...
    USERAUTH_SUCCESS_PACKET = new Buffer([MESSAGE.USERAUTH_SUCCESS]),
    REQUEST_SUCCESS_PACKET = new Buffer([MESSAGE.REQUEST_SUCCESS]),
    REQUEST_FAILURE_PACKET = new Buffer([MESSAGE.REQUEST_FAILURE]),
...
function SSH2Stream(cfg) {
  if (!(this instanceof SSH2Stream))
    return new SSH2Stream(cfg);
  ...
  if (this.config.ident.length > 255)
  ...
  this.on('header', function(header) {
    if (header.versions.protocol !== '1.99'
      ...
      self.disconnect(DISCONNECT_REASON.PROTOCOL_VERSION_NOT_SUPPORTED);
      return;
    }
...
SSH2Stream.prototype._transform = function(chunk, encoding, callback) {
  var skipDecrypt = false,
      doDecryptGCM = false,
      state = this._state,
      instate = state.incoming,
      expect = instate.expect,
      decrypt = instate.decrypt,
      chlen = chunk.length,
      chleft = 0,
      debug = this.debug,
      i = 0,
  ...
  while (true) {
    if (expect.type !== undefined) {
      ...
      if (expect.type === EXP_TYPE_BYTES) {
        ...
        if (++expect.ptr > 255) {
          this.reset();
          debug('DEBUG: Parser: Greeting exceeded 255 characters');
          return callback(new Error('Greeting size limit exceeded'));
        }
        if (chunk[i] === 0x0A) {
          expect.type = undefined;
          if (p < i) {
    ...
    if (instate.status === IN_INIT) {
      if (this.server) {
        ...
        if (!buffer) {
          debug('DEBUG: Parser: IN_INIT (waiting for header begin)');
          ...
          if (buffer[0] === 0x53       // S
            ...
            debug('DEBUG: Parser: IN_INIT (waiting for header remainder)');
          } else {
            this.reset();
            debug('DEBUG: Parser: Bad protocol version exchange start');
            return callback(new Error('Bad protocol version exchange start'));
          }
        }
      } else {
        debug('DEBUG: Parser: IN_INIT');
        // retrieve any bytes that may come before the protocol version exchange
        var ss = instate.search = new StreamSearch(new Buffer('SSH-'));
      ...
      // packet length
      //expectData(this, EXP_TYPE_BYTES, decrypt.size, '_decryptBuf');
      ...
      instate.pktLen = buffer.readUInt32BE(0, true);
      ...
      if (instate.hmac.size !== undefined) {
        ...
        //expectData(this, EXP_TYPE_BYTES, instate.hmac.size, '_hmacBuf');
      ...
      if (debug !== DEBUG_NOOP) {
        var msgPacket = 'DEBUG: Parser: IN_PACKETDATAAFTER, packet: ',
            ...
            ptype = instate.payload[0],
            msgPktType = undefined;
...
SSH2Stream.prototype.ping = function() {
  this.debug('DEBUG: Outgoing: Writing ping (MESSAGE_IGNORE)');
...
SSH2Stream.prototype.channelEOF = function(chan) {
  ...
  this.debug('DEBUG: Outgoing: Writing CHANNEL_EOF');
...
SSH2Stream.prototype.channelClose = function(chan) {
  ...
  this.debug('DEBUG: Outgoing: Writing CHANNEL_CLOSE');
  return send(this, buf);
};
...
SSH2Stream.prototype.channelData = function(chan, data) {
  var dataIsBuffer = Buffer.isBuffer(data),
      dataLen = (dataIsBuffer ? data.length : Buffer.byteLength(data)),
      buf = new Buffer(1 + 4 + 4 + dataLen);
  ...
  this.debug('DEBUG: Outgoing: Writing CHANNEL_DATA');
  return send(this, buf);
};
SSH2Stream.prototype.channelExtData = function(chan, data, type) {
  var dataIsBuffer = Buffer.isBuffer(data),
      dataLen = (dataIsBuffer ? data.length : Buffer.byteLength(data)),
      buf = new Buffer(1 + 4 + 4 + 4 + dataLen);
  ...
  this.debug('DEBUG: Outgoing: Writing CHANNEL_EXTENDED_DATA');
  return send(this, buf);
};
SSH2Stream.prototype.channelOpenConfirm = function(remoteChan, localChan,
  ...
  this.debug('DEBUG: Outgoing: Writing CHANNEL_OPEN_CONFIRMATION');
  return send(this, buf);
};
SSH2Stream.prototype.channelOpenFail = function(remoteChan, reason, desc, lang) {
  ...
  var descLen = Buffer.byteLength(desc),
      langLen = Buffer.byteLength(lang),
      p = 9,
      buf = new Buffer(1 + 4 + 4 + 4 + descLen + 4 + langLen);
...
SSH2Stream.prototype.directTcpip = function(chan, initWindow, maxPacket, cfg) {
  ...
  var srclen = Buffer.byteLength(cfg.srcIP),
      dstlen = Buffer.byteLength(cfg.dstIP),
      p = 29,
      buf = new Buffer(1 + 4 + 12 + 4 + 4 + 4 + 4 + srclen + 4 + 4 + dstlen + 4);
  ...
  this.debug('DEBUG: Outgoing: Writing CHANNEL_OPEN (direct-tcpip)');
  return send(this, buf);
};
SSH2Stream.prototype.session = function(chan, initWindow, maxPacket) {
  ...
  this.debug('DEBUG: Outgoing: Writing CHANNEL_OPEN (session)');
  return send(this, buf);
};
SSH2Stream.prototype.agentForward = function(chan, wantReply) {
  if (this.server)
    throw new Error('Client-only method called in server mode');
  ...
  this.debug('DEBUG: Outgoing: Writing CHANNEL_REQUEST (auth-agent-req@openssh.com)');
  return send(this, buf);
};
SSH2Stream.prototype.windowChange = function(chan, rows, cols, height, width) {
  ...
  this.debug('DEBUG: Outgoing: Writing CHANNEL_REQUEST (window-change)');
  return send(this, buf);
};
SSH2Stream.prototype.pty = function(chan, rows, cols, height,
  ...
  this.debug('DEBUG: Outgoing: Writing CHANNEL_REQUEST (pty-req)');
  return send(this, buf);
};
SSH2Stream.prototype.shell = function(chan, wantReply) {
  ...
  this.debug('DEBUG: Outgoing: Writing CHANNEL_REQUEST (shell)');
  return send(this, buf);
};
SSH2Stream.prototype.exec = function(chan, cmd, wantReply) {
  ...
  this.debug('DEBUG: Outgoing: Writing CHANNEL_REQUEST (exec)');
  return send(this, buf);
};
SSH2Stream.prototype.signal = function(chan, signal) {
  ...
  this.debug('DEBUG: Outgoing: Writing CHANNEL_REQUEST (signal)');
  return send(this, buf);
};
SSH2Stream.prototype.env = function(chan, key, val, wantReply) {
  ...
  var keyLen = Buffer.byteLength(key),
      valLen = (Buffer.isBuffer(val) ? val.length : Buffer.byteLength(val)),
      buf = new Buffer(1 + 4 + 4 + 3 + 1 + 4 + keyLen + 4 + valLen);
  ...
  this.debug('DEBUG: Outgoing: Writing CHANNEL_REQUEST (env)');
  return send(this, buf);
};
SSH2Stream.prototype.x11Forward = function(chan, cfg, wantReply) {
  ...
  this.debug('DEBUG: Outgoing: Writing CHANNEL_REQUEST (x11-req)');
  return send(this, buf);
};
SSH2Stream.prototype.subsystem = function(chan, name, wantReply) {
  ...
  var nameLen = Buffer.byteLength(name),
      buf = new Buffer(1 + 4 + 4 + 9 + 1 + 4 + nameLen);
...
SSH2Stream.prototype.authPassword = function(username, password) {
  ...
  var userLen = Buffer.byteLength(username),
      passLen = Buffer.byteLength(password),
      p = 0,
      buf = new Buffer(1
                       + 4 + userLen
                       + 4 + 14 // "ssh-connection"
...
SSH2Stream.prototype.authPK = function(username, pubKey, cbSign) {
  ...
  var userLen = Buffer.byteLength(username),
      ...
      sesLen = outstate.sessionid.length,
      p = 0,
      buf = new Buffer((cbSign ? 4 + sesLen : 0)
                       + 1
                       + 4 + userLen
                       + 4 + 14 // "ssh-connection"
                       + 4 + 9 // "publickey"
...
SSH2Stream.prototype.authInfoRes = function(responses) {
  ...
  if (responses) {
    p += 4;
    for (var i = 0, len = responses.length, resLen; i < len; ++i) {
...
SSH2Stream.prototype.forwardedTcpip = function(chan, initWindow, maxPacket,
                                               bindAddr, bindPort, remoteAddr,
                                               remotePort) {
  if (!this.server)
    throw new Error('Server-only method called in client mode');

  var bindAddrLen = Buffer.byteLength(bindAddr),
      remoteAddrLen = Buffer.byteLength(remoteAddr),
      p = 36 + bindAddrLen,
      buf = new Buffer(1 + 4 + 15 + 4 + 4 + 4 + 4 + bindAddrLen + 4 + 4
                       + remoteAddrLen + 4);
  ...
  buf.writeUInt32BE(bindAddrLen, 32, true);
  buf.write(bindAddr, 36, bindAddrLen, 'ascii');

  buf.writeUInt32BE(bindPort, p, true);

  buf.writeUInt32BE(remoteAddrLen, p += 4, true);
  buf.write(remoteAddr, p += 4, remoteAddrLen, 'ascii');

  buf.writeUInt32BE(remotePort, p += remoteAddrLen, true);

  this.debug('DEBUG: Outgoing: Writing CHANNEL_OPEN (forwarded-tcpip)');
  return send(this, buf);
};
SSH2Stream.prototype.channelSuccess = function(chan) {
  ...
  this.debug('DEBUG: Outgoing: Writing CHANNEL_SUCCESS');
  return send(this, buf);
};
SSH2Stream.prototype.channelFailure = function(chan) {
  ...
  this.debug('DEBUG: Outgoing: Writing CHANNEL_FAILURE');
  return send(this, buf);
};
SSH2Stream.prototype.exitStatus = function(chan, status) {
  ...
  this.debug('DEBUG: Outgoing: Writing CHANNEL_REQUEST (exit-status)');
  return send(this, buf);
};
SSH2Stream.prototype.exitSignal = function(chan, name, coreDumped, msg) {
  ...
  // does not consume window space
  var nameLen = Buffer.byteLength(name),
      msgLen = (msg ? Buffer.byteLength(msg) : 0),
      p = 25 + nameLen,
      buf = new Buffer(1 + 4 + 4 + 11 + 1 + 4 + nameLen + 1 + 4 + msgLen + 4);
  ...
  this.debug('DEBUG: Outgoing: Writing CHANNEL_REQUEST (exit-signal)');
  return send(this, buf);
};
SSH2Stream.prototype.x11 = function(chan, initWindow, maxPacket, originAddr,
                                    originPort) {
  if (!this.server)
    throw new Error('Server-only method called in client mode');

  var addrLen = Buffer.byteLength(originAddr),
      p = 24 + addrLen,
      buf = new Buffer(1 + 4 + 3 + 4 + 4 + 4 + 4 + addrLen + 4);
  ...
  buf.write(originAddr, 24, addrLen, 'ascii');

  buf.writeUInt32BE(originPort, p, true);

  this.debug('DEBUG: Outgoing: Writing CHANNEL_OPEN (x11)');
  return send(this, buf);
};
// ssh-userauth service-specific
SSH2Stream.prototype.authFail = function(authMethods, isPartial) {
  if (!this.server)
    throw new Error('Server-only method called in client mode');
  ...
  var methodsLen = methods.length,
      buf = new Buffer(1 + 4 + methodsLen + 1);
...
SSH2Stream.prototype.authPKOK = function(keyAlgo, key) {
  ...
  var keyAlgoLen = keyAlgo.length,
      keyLen = key.length,
      buf = new Buffer(1 + 4 + keyAlgoLen + 4 + keyLen);
...
SSH2Stream.prototype.authPasswdChg = function(prompt, lang) {
  ...
  var promptLen = Buffer.byteLength(prompt),
      langLen = lang ? lang.length : 0,
      p = 0,
      buf = new Buffer(1 + 4 + promptLen + 4 + langLen);
...
SSH2Stream.prototype.authInfoReq = function(name, instructions, prompts) {
  ...
  var promptsLen = 0,
      nameLen = name ? Buffer.byteLength(name) : 0,
      instrLen = instructions ? Buffer.byteLength(instructions) : 0,
      p = 0;
  ...
  p += 4;
  for (var i = 0, len = prompts.length, prompt, promptLen; i < len; ++i) {
    prompt = prompts[i];
...
function onKEXINIT(self, init) { // client/server
  var state = self._state,
      ...
      waitForSend = (outstate.kexinit === undefined);
  ...
  if (waitForSend)
    self.once('_KEXINIT_WRITTEN', doCheck);
  else
    doCheck();

  function doCheck() {
    if (check_KEXINIT(self, init) === true && !self.server) {
...
function check_KEXINIT(self, init) {
  ...
  debug('DEBUG: (remote) Server->Client ciphers: ' + (init.algorithms.sc.encrypt));
  // check for agreeable server->client cipher
  for (i = 0, len = ALGORITHMS.CIPHER.length;
       i < len && init.algorithms.sc.encrypt.indexOf(ALGORITHMS.CIPHER[i]) === -1;
       ++i);
  if (i === len) {
  ...
  if (self.server)
    val = outstate.encrypt.type = ALGORITHMS.CIPHER[i];
  else
    val = instate.decrypt.type = ALGORITHMS.CIPHER[i];
  debug('DEBUG: Server->Client Cipher: ' + val);

  debug('DEBUG: (local) Client->Server ciphers: ' + ALGORITHMS.CIPHER);
  debug('DEBUG: (remote) Client->Server ciphers: ' + init.algorithms.cs.encrypt);
  // check for agreeable client->server cipher
  for (i = 0, len = ALGORITHMS.CIPHER.length;
       i < len && init.algorithms.cs.encrypt.indexOf(ALGORITHMS.CIPHER[i]) === -1;
       ++i);
  if (i === len) {
    // no suitable match found!
  ...
  debug('DEBUG: (local) KEX algorithms: ' + ALGORITHMS.KEX);
  ...
  if (self.server)
    val = instate.hmac.type = ALGORITHMS.HMAC[i];
  else
    val = outstate.hmac.type = ALGORITHMS.HMAC[i];
  ...
  for (i = 0, len = ALGORITHMS.COMPRESS.length;
       i < len && init.algorithms.cs.compress.indexOf(ALGORITHMS.COMPRESS[i]) === -1;
       ++i);
  if (i === len) {
    // no suitable match found!
  ...
  if (self.server)
    val = instate.compress.type = ALGORITHMS.COMPRESS[i];
  else
    val = outstate.compress.type = ALGORITHMS.COMPRESS[i];
  ...
  for (i = 0, len = ALGORITHMS.COMPRESS.length;
       i < len && init.algorithms.sc.compress.indexOf(ALGORITHMS.COMPRESS[i]) === -1;
       ++i);
  if (i === len) {
    // no suitable match found!
  ...
  state.hostkeyFormat = ALGORITHMS.SERVER_HOST_KEY[i];
  ...
  if (state.kex) {
    ...
    if (outstate.pubkey[0] & 0x80) {
      var key = new Buffer(outstate.pubkey.length + 1);
      key[0] = 0;
      outstate.pubkey.copy(key, 1);
      outstate.pubkey = key;
    }
...
function onKEXDH_REPLY(self, info) { // client
  var state = self._state,
      instate = state.incoming,
      outstate = state.outgoing,
      debug = self.debug,
  ...
  if (info.sig_format !== readString(info.sig, 0, 'ascii')) {
  ...
  if (outstate.status === OUT_INIT) {
    ...
    var verifiedHost;
    function verifyHost(permitted) {
      if (verifiedHost !== undefined)
        return;
      verifiedHost = permitted;
    }
    self.emit('fingerprint', info.hostkey, verifyHost);
    if (verifiedHost === undefined)
  ...
  var len_ident = Buffer.byteLength(self.config.ident),
      len_sident = Buffer.byteLength(instate.identRaw),
      len_init = outstate.kexinit.length,
      len_sinit = instate.kexinit.length,
      len_hostkey = info.hostkey.length,
      len_gex_req = 0,
      len_gex_prime = 0,
      len_gex_gen = 0,
      len_pubkey = outstate.pubkey.length,
  ...
  bp += len_pubkey - (outstate.pubkey[0] & 0x80 ? 1 : 0);
  ...
  var asnWriter = new Ber.Writer(),
      rawsig = readString(info.sig, info.sig._pos), // s
  ...
  if (algo === 'RSA') {
    var e = readString(info.hostkey, info.hostkey._pos),
        n = readString(info.hostkey, info.hostkey._pos);
    asnWriter.startSequence();
    ...
    var p = readString(info.hostkey, info.hostkey._pos),
        q = readString(info.hostkey, info.hostkey._pos),
        g = readString(info.hostkey, info.hostkey._pos),
        y = readString(info.hostkey, info.hostkey._pos);
...
function onNEWKEYS(self) { // client/server
  ...
  var blocklen = 8,
      keylen = 0,
      p = 0,
      dhHashAlgo = (RE_SHA1.test(state.kexdh) ? 'sha1' : 'sha256'),
      len_secret = (outstate.kexsecret[0] & 0x80 ? 1 : 0)
                   + outstate.kexsecret.length,
  ...
  if (outstate.compress.type !== 'none'/* && outstate.compress.instance*/)
    outstate.compress.instance = zlib.createDeflate();
  if (instate.compress.type !== 'none')
    instate.compress.instance = zlib.createInflate();
  ...
  if (outstate.status === OUT_REKEYING) {
    ...
    for (var q = 0, qlen = queue.length; q < qlen; ++q) {
...
function parsePacket(self) {
  var instate = self._state.incoming,
      outstate = self._state.outgoing,
      payload = instate.payload,
      seqno = instate.seqno,
      serviceName,
      lang,
      message,
  ...
  if (type === MESSAGE.CHANNEL_DATA) {
    /*
      ...
      string    data
    */
    self.emit('CHANNEL_DATA:' + payload.readUInt32BE(1, true),
    ...
    /*
      ...
      string    data
    */
    self.emit('CHANNEL_EXTENDED_DATA:' + payload.readUInt32BE(1, true),
    ...
    /*
      ...
      uint32    bytes to add
    */
    self.emit('CHANNEL_WINDOW_ADJUST:' + payload.readUInt32BE(1, true),
              payload.readUInt32BE(5, true));
    ...
    /*
      ...
      uint32    recipient channel
    */
    self.emit('CHANNEL_SUCCESS:' + payload.readUInt32BE(1, true));
  } else if (type === MESSAGE.CHANNEL_FAILURE) {
    /*
      byte      SSH_MSG_CHANNEL_FAILURE
      uint32    recipient channel
    */
    self.emit('CHANNEL_FAILURE:' + payload.readUInt32BE(1, true));
  } else if (type === MESSAGE.CHANNEL_EOF) {
    /*
      byte      SSH_MSG_CHANNEL_EOF
      uint32    recipient channel
    */
    self.emit('CHANNEL_EOF:' + payload.readUInt32BE(1, true));
    ...
    */
    ...
    if (chanType === 'forwarded-tcpip') {
      ...
      */
      ...
      self.emit('CHANNEL_OPEN', channel);
    } else { // 'session' and 'auth-agent@openssh.com'
      channel = {
        type: chanType,
        sender: sender,
        window: window,
    ...
    */
    ...
    /*
      ...
      uint32    recipient channel
    */
    self.emit('CHANNEL_CLOSE:' + payload.readUInt32BE(1, true));
    ...
    */
    var reason = payload.readUInt32BE(1, true),
        reasonText = DISCONNECT_REASON[reason],
  ...
  } else if (type === MESSAGE.KEXDH_INIT) {
    /*
      byte      SSH_MSG_KEXDH_INIT
      mpint     e
    */
    var e = readString(payload, 1);
    ...
    */
    ...
    /*
      ...
      string    service name
    */
    serviceName = readString(payload, 1, 'ascii');
    ...
    */
    var username = readString(payload, 1, 'utf8'),
        svcName = readString(payload, payload._pos, 'ascii'),
        method = readString(payload, payload._pos, 'ascii'),
        methodData;

    if (method === 'password')
      methodData = readString(payload, payload._pos + 1, 'utf8');
    else if (method === 'publickey') {
      var pkSigned = payload[payload._pos++],
          keyAlgo = readString(payload, payload._pos, 'ascii'),
          key = readString(payload, payload._pos),
          signature,
          blob;

      if (pkSigned) {
        signature = readString(payload, payload._pos);
        blob = new Buffer(4 + outstate.sessionId.length + payload._pos);
        blob.writeUInt32BE(outstate.sessionId.length, 0, true);
        outstate.sessionId.copy(blob, 4);
        payload.copy(blob, 4 + outstate.sessionId.length, 0, payload._pos);
      }
      ...
      var skipLen = payload.readUInt32BE(payload._pos, true);
      methodData = readString(payload, payload._pos + 4 + skipLen, 'utf8');
    } else if (method !== 'none')
    ...
    */
    var request = readString(payload, 1, 'ascii'),
        wantReply = (payload[payload._pos++] !== 0),
        reqData;

    if (request === 'tcpip-forward' || request === 'cancel-tcpip-forward') {
      reqData = {
        bindAddr: readString(payload, payload._pos, 'ascii'),
        bindPort: payload.readUInt32BE(payload._pos, true)
      };
    } else
...
function parse_KEX(self, type) {
  ...
  if (state.kexdh !== 'group') {
    ...
    if (type === MESSAGE.KEXDH_GEX_REQUEST) {
      // TODO
    } else if (type === MESSAGE.KEXDH_GEX_GROUP) {
      /*
...
function parse_KEXDH_REPLY(self) {
...
function parse_USERAUTH(self, type) {
  var state = self._state,
      authMethod = state.authMethod,
      payload = state.incoming.payload,
      message,
      lang;
    ...
    if (type === MESSAGE.USERAUTH_INFO_REQUEST) {
      ...
      */
      ...
      if (nprompts > 0) {
        ...
        self.emit('USERAUTH_INFO_REQUEST', name, instr, lang);
      ...
      */
      var nresponses = payload.readUInt32BE(1, true);
      if (nresponses > 0) {
        var responses = [];
        for (var response = 0; response < nresponses; ++response)
          responses.push(readString(payload, payload._pos, 'utf8'));
        self.emit('USERAUTH_INFO_RESPONSE', responses);
      } else
        self.emit('USERAUTH_INFO_RESPONSE');
    }
  } else if (authMethod === 'pubkey') {
...
function parse_CHANNEL_REQUEST(self) {
  var payload = self._state.incoming.payload,
      info;
  ...
  if (request === 'exit-status') { // server->client
    ...
    */
    info = {
      recipient: recipient,
      request: request,
      code: payload.readUInt32BE(++payload._pos, true)
    };
    ...
    */
    info = {
      recipient: recipient,
      request: request,
      signal: readString(payload, ++payload._pos, 'ascii'),
    ...
    */
    info = {
      ...
      request: request,
      wantReply: (payload[payload._pos++] !== 0),
      term: readString(payload, payload._pos, 'ascii'),
      cols: payload.readUInt32BE(payload._pos, true),
      rows: payload.readUInt32BE(payload._pos += 4, true),
      width: payload.readUInt32BE(payload._pos += 4, true),
      height: payload.readUInt32BE(payload._pos += 4, true),
      modes: readString(payload, payload._pos += 4)
    };
    ...
    */
    info = {
      ...
      request: request,
      wantReply: (payload[payload._pos++] !== 0),
      single: (payload[payload._pos++] !== 0),
      protocol: readString(payload, payload._pos, 'ascii'),
      cookie: readString(payload, payload._pos),
      screen: payload.readUInt32BE(payload._pos, true)
    };
    ...
    */
    info = {
      ...
      request: request,
      wantReply: (payload[payload._pos++] !== 0),
      key: readString(payload, payload._pos, 'utf8'),
      val: readString(payload, payload._pos, 'utf8')
    };
    ...
    */
    info = {
      ...
      request: request,
      wantReply: (payload[payload._pos++] !== 0),
      command: readString(payload, payload._pos, 'utf8')
    };
    ...
    */
    info = {
      ...
      request: request,
      wantReply: (payload[payload._pos++] !== 0),
      subsystem: readString(payload, payload._pos, 'utf8')
    };
    ...
    */
    info = {
      ...
      request: request,
      wantReply: false,
      signal: 'SIG' + readString(payload, ++payload._pos, 'ascii')
    };
    ...
    */
    info = {
      ...
      request: request,
      wantReply: false,
      clientControl: (payload[++payload._pos] !== 0)
    };
  } else {
...
function hmacVerify(self, data) {
  ...
  if (isGCM(instate.decrypt.type)) {
    var decrypt = instate.decrypt,
        instance = decrypt.instance;

    instance.setAuthTag(data);

    var payload = new Buffer(instance.update(instate.packet, 'binary', 'binary'),
                             'binary');
...
function KEXINIT(self) { // client/server
  ...
  */
  ...
  buf.writeUInt32BE(ALGORITHMS.KEX_LIST_SIZE, p, true);
  p += 4;
  ALGORITHMS.KEX_LIST.copy(buf, p);
  p += ALGORITHMS.KEX_LIST_SIZE;

  buf.writeUInt32BE(ALGORITHMS.SERVER_HOST_KEY_LIST_SIZE, p, true);
  p += 4;
  ALGORITHMS.SERVER_HOST_KEY_LIST.copy(buf, p);
  ...
  buf.writeUInt32BE(ALGORITHMS.COMPRESS_LIST_SIZE, p, true);
  p += 4;
  ALGORITHMS.COMPRESS_LIST.copy(buf, p);
  ...
  if (outstate.status === OUT_REKEYING)
    ret = send(self, buf, undefined, true);
  else
    ret = send(self, buf);

  self.emit('_KEXINIT_WRITTEN');
...
function KEXDH_REPLY(self, e) { // server
  var state = self._state,
      outstate = state.outgoing,
      instate = state.incoming,
      hostkey = self.config.publicKey.public,
      hostkeyAlgo = self.config.publicKey.fulltype,
      privateKey = self.config.privateKey.privateOrig;
  ...
  var compSecret = state.kex.computeSecret(e, 'binary', 'binary'),
      secret = new Buffer(compSecret, 'binary'),
      hash = crypto.createHash(RE_SHA1.test(state.kexdh)
                               ? 'sha1'
                               : 'sha256');
  var len_ident = Buffer.byteLength(instate.identRaw),
      len_sident = Buffer.byteLength(self.config.ident),
      len_init = instate.kexinit.length,
      len_sinit = outstate.kexinit.length,
      len_hostkey = hostkey.length,
      len_gex_req = 0,
      len_gex_prime = 0,
      len_gex_gen = 0,
      len_pubkey = e.length,
      len_spubkey = outstate.pubkey.length,
      len_secret = secret.length;
  var gex_prime, gex_gen;
  if (state.kexdh !== 'group') {
    len_gex_req = 12;
    gex_prime = state.kex.getPrime();
  ...
  bp += len_spubkey - (outstate.pubkey[0] & 0x80 ? 1 : 0);
  ...
  var algo = (hostkeyAlgo === 'ssh-rsa' ? 'RSA' : 'DSA'),
      signer = crypto.createSign(algo + '-SHA1'),
      signature;
...
function KEXDH_GEX_REQ(self) { // client
  ...
  if (self._state.outgoing.status === OUT_REKEYING)
    return send(self, KEXDH_GEX_REQ_PACKET, undefined, true);
  else
    return send(self, KEXDH_GEX_REQ_PACKET);
}
...
function send(self, payload, cb, bypass) {
  ...
  if (encrypt.instance !== false) {
    if (isGCM(encrypt.type)) {
      ...
      self.push(encrypter.update(buf.slice(4), 'binary', 'binary'), 'binary');

      var final = encrypter.final('binary');
      if (final.length)
        self.push(final, 'binary');

      ret = self.push(encrypter.getAuthTag());

      iv_inc(encrypt.iv);
    } else {
      self.push(encrypt.instance.update(buf, 'binary', 'binary'), 'binary');
      ret = self.push(mac);
    }
  } else
    ret = self.push(buf);
...
function readString(buffer, start, encoding) {
  ...
  if (encoding) {
    if (Buffer.isBuffer(encoding)) {
      buffer.copy(encoding, 0, start + 4, start + 4 + slen);
      return encoding;
    } else
