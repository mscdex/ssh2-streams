...
function SSH2Stream(cfg) {
  ...
  if (this.server
      ...
      && typeof cfg.privateKey !== 'object')
    throw new Error('Invalid/missing privateKey');
  else if (this.server) {
    function parsePubPrivKey(privateKey, passphrase) {
      ...
      if (privKeyInfo.encryption) {
        ...
        decryptKey(privKeyInfo, passphrase);
      }
      publicKeyInfo = genPublicKey(privKeyInfo);
      return { privateKey: privKeyInfo, publicKey: publicKeyInfo };
    }
    if (typeof cfg.privateKey !== 'string'
      && !Buffer.isBuffer(cfg.privateKey)) {
      Object.keys(cfg.privateKey).forEach(function(key) {
        if(cfg.passphrase) {
          var info = parsePubPrivKey(cfg.privateKey[key], cfg.passphrase[key]);
        } else {
          var info = parsePubPrivKey(cfg.privateKey[key]);
        }
        self.config.privateKey[info.publicKey.fulltype] = info.privateKey;
        self.config.publicKey[info.publicKey.fulltype] = info.publicKey;

      });
    } else {
      var info = parsePubPrivKey(cfg.privateKey, cfg.passphrase);
      this.config.privateKey[info.publicKey.fulltype] = info.privateKey;
      this.config.publicKey[info.publicKey.fulltype] = info.publicKey;
    }
  }

  // client/server
  this.config.ident = 'SSH-2.0-'
...
SSH2Stream.prototype.directTcpip = function(chan, initWindow, maxPacket, cfg) {
  ...
  var srclen = Buffer.byteLength(cfg.srcIP),
      ...
      p = 29,
      buf = new Buffer(1 + 4 + 12 + 4 + 4 + 4 + 4 + srclen + 4 + 4 + dstlen
                       + 4);
...
SSH2Stream.prototype.x11Forward = function(chan, cfg, wantReply) {
  ...
  var protolen = Buffer.byteLength(cfg.protocol),
      ...
      buf = new Buffer(1 + 4 + 4 + 7 + 1 + 1 + 4 + protolen + 4 + cookielen
                       + 4);
...
function check_KEXINIT(self, init) {
  ...
  if (self.server) {
    serverList = Object.keys(self.config.publicKey);
...
function onNEWKEYS(self) { // client/server
  ...
  var blocklen = 8,
      ...
      len_secret = (outstate.kexsecret[idx_secret] & 0x80 ? 1 : 0) + len,
...
function KEXINIT(self) { // client/server
  ...
  */
  ...
  var hostKeyList;
  if (self.server) {
    hostKeyList = Object.keys(self.config.publicKey).join(',');
  } else
...
function KEXDH_REPLY(self, e) { // server
  var state = self._state,
      outstate = state.outgoing,
      instate = state.incoming,
      hostkey = self.config.publicKey[state.hostkeyFormat].public,
      hostkeyAlgo = self.config.publicKey[state.hostkeyFormat].fulltype,
      privateKey = self.config.privateKey[state.hostkeyFormat].privateOrig;
