...
function onKEXDH_REPLY(self, info, callback) { // client
  ...
  var asn1KeyBuf;
  if (keyAlgo === 'RSA-SHA1') {
    asn1KeyBuf = RSAKeySSHToASN1(info.hostkey, self, callback);
  } else if (keyAlgo === 'DSA-SHA1') {
    asn1KeyBuf = DSAKeySSHToASN1(info.hostkey, self, callback);
    rawsig = DSASigBareToBER(rawsig);
  } else {
    // ECDSA
    asn1KeyBuf = ECDSAKeySSHToASN1(info.hostkey, self, callback);
    rawsig = ECDSASigSSHToASN1(rawsig, self, callback);
  }
