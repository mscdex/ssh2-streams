...
SSH2Stream.prototype.openssh_StreamLocalForward = function(socketPath, wantReply) {
  ...
  var pathlen = Buffer.byteLength(pathlen),
      buf = new Buffer(1 + 4 + 31 + 1 + 4 + pathlen);
...
SSH2Stream.prototype.openssh_CancelStreamLocalForward = function(socketPath,
  ...
  var pathlen = Buffer.byteLength(pathlen),
      buf = new Buffer(1 + 4 + 38 + 1 + 4 + pathlen);
...
SSH2Stream.prototype.openssh_directStreamLocal = function(chan, initWindow,
  ...
  var pathlen = Buffer.byteLength(cfg.socketPath),
      p = 47,
      buf = new Buffer(1 + 4 + 30 + 4 + 4 + 4 + 4 + pathlen + 4);
  ...
  buf.writeUInt32BE(0, p += pathlen, true);

  this.debug('DEBUG: Outgoing: Writing CHANNEL_OPEN (direct-streamlocal@openssh.com)');
  return send(this, buf);
};
...
SSH2Stream.prototype.openssh_forwardedStreamLocal = function(chan, initWindow,
  ...
  var pathlen = Buffer.byteLength(socketPath),
      buf = new Buffer(1 + 4 + 33 + 4 + 4 + 4 + 4 + pathlen + 4);
  ...
  buf.write(socketPath, 54, pathlen, 'utf8');

  buf.writeUInt32BE(0, 54 + pathlen, true);

  this.debug('DEBUG: Outgoing: Writing CHANNEL_OPEN (forwarded-streamlocal@openssh.com)');
  return send(this, buf);
};
...
function parsePacket(self, callback) {
  ...
  if (type === MESSAGE.CHANNEL_DATA) {
    ...
    */
    ...
    if (chan === false)
      return false;
    data = readString(payload, 5, self, callback);
    if (data === false)
      return false;
    ...
    */
    ...
    } else if (chanType === 'forwarded-streamlocal@openssh.com'
    ...
    */
    ...
    if (reasonCode === false)
      return false;
    description = readString(payload, payload._pos, 'utf8', self, callback);
