...
function check_KEXINIT(self, init, firstFollows) {
  ...
  switch (kex_algorithm) {
    ...
    case 'diffie-hellman-group14-sha256':
      state.kexdh = 'group256';
      state.kex = crypto.getDiffieHellman('modp14');
      break;
    case 'diffie-hellman-group15-sha512':
      state.kexdh = 'group512';
      state.kex = crypto.getDiffieHellman('modp15');
      break;
    case 'diffie-hellman-group16-sha512':
      state.kexdh = 'group512';
      state.kex = crypto.getDiffieHellman('modp16');
      break;
    case 'diffie-hellman-group17-sha512':
      state.kexdh = 'group512';
      state.kex = crypto.getDiffieHellman('modp17');
      break;
    case 'diffie-hellman-group18-sha512':
      state.kexdh = 'group512';
      state.kex = crypto.getDiffieHellman('modp18');
      break;
...
function onKEXDH_REPLY(self, info, verifiedHost) { // Client
  ...
  var hashAlgo;
  if (state.kexdh.indexOf('group') !== -1) {
    var hashType = state.kexdh.match(/group(\d*)/)[1];
    hashAlgo = hashType.length > 0 ? `sha${hashType}` : 'sha1';
  }
  else
...
function onNEWKEYS(self) { // Client/Server
  ...
  if (state.kexdh.indexOf('group') !== -1) {
    var hashType = state.kexdh.match(/group(\d*)/)[1];
    dhHashAlgo = hashType.length > 0 ? `sha${hashType}` : 'sha1';
  }
  else
