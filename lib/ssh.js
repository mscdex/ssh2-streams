...
SSH2Stream.prototype.authPK = function(username, pubKey, cbSign) {
  ...
  cbSign(buf, function(signature) {
    if (pubKeyFullType !== 'ssh-rsa' && pubKeyFullType !== 'ssh-dss') {
      // ECDSA
      if (signature[0] !== 0x00) {
        // Convert r and s values from asn.1 to SSH format
        ...
        // TODO: DRY these sorts of conversions
...
function onKEXDH_REPLY(self, info, callback) { // client
  ...
  if (keyAlgo === 'RSA-SHA1') {
    ...
    var curve = readString(info.hostkey, info.hostkey._pos, self, callback);
    if (curve === false)
      return false;
    ...
    asnWriter.startSequence();
      ...
      asnWriter.writeOID('1.2.840.10045.3.1.7'); // prime256v1/secp256r1
    asnWriter.endSequence();
    ...
    var r = readString(rawsig, 0, self, callback);
    if (r === false)
      return false;

    var s = readString(rawsig, rawsig._pos, self, callback);
    if (s === false)
      return false;

    asnSigWriter = new Ber.Writer();
    asnSigWriter.startSequence();
      asnSigWriter.writeBuffer(r, Ber.Integer);
      asnSigWriter.writeBuffer(s, Ber.Integer);
    asnSigWriter.endSequence();
    rawsig = asnSigWriter.buffer;
  }
  asnWriter.endSequence();
  ...
  randBytes(16, function(myCookie) {
    ...
    */
    ...
    if (self.server) {
      ...
      hostKeyBuf = self.config.publicKey.fulltype;
    } else
      hostKeyBuf = algos.serverHostKeyBuf;
    ...
    p += 4;
    if (typeof hostKeyBuf === 'string')
      buf.write(hostKeyBuf, p);
    else
...
function KEXDH_REPLY(self, e) { // server
  ...
  if (keyAlgo === 'DSA-SHA1') {
    ...
    // 40 bytes total
    asn1Reader = new Ber.Reader(signature);
    var zeros;
    ...
    if (zeros > 0)
      s = s.slice(zeros);
    newSig = new Buffer(r.length + s.length);
    r.copy(newSig, 0);
    s.copy(newSig, r.length);
    signature = newSig;
    ...
    r = asn1Reader.readString(Ber.Integer, true);
    s = asn1Reader.readString(Ber.Integer, true);
    newSig = new Buffer(4 + r.length + 4 + s.length);
    var p = 0;
    newSig.writeUInt32BE(r.length, p, true);
    r.copy(newSig, p += 4);
    newSig.writeUInt32BE(s.length, p += r.length, true);
  ...
  */
  ...
  bp += 4;
  signature.copy(buf, bp); // signature
