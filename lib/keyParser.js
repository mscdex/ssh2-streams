...
module.exports = function(data) {
  ...
  if ((m = RE_HEADER_OPENSSH_PRIV.exec(data[0]))
    ...
    ret.private = new Buffer(m[5].replace(/\r?\n/g, ''), 'base64');

    // TODO: verify private key MAC
    /*var mac = new Buffer(m[6].replace(/\r?\n/g, ''), 'hex'),
        mackey = 'putty-private-key-file-mac-key',
        typelen = ret.fulltype.length,
        enclen = m[2].length,
        commlen = Buffer.byteLength(ret.comment.length),
        publen = ret.public.length,
        privlen = ret.private.length,
    ...
    macdata.write(ret.fulltype, p += 4, typelen, 'ascii');
    macdata.writeUInt32BE(enclen, p += typelen, true);
    macdata.write(m[2], p += 4, enclen, 'ascii');
    macdata.writeUInt32BE(commlen, p += enclen, true);
    macdata.write(ret.comment, p += 4, commlen, 'utf8');
    macdata.writeUInt32BE(publen, p += commlen, true);
    ret.public.copy(macdata, p += 4);
    macdata.writeUInt32BE(privlen, p += publen, true);
    ret.private.copy(macdata, p += 4);
    */
