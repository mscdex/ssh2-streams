...
var genOpenSSLRSAPriv = (function() {
  ...
  try {
    ...
    return new Function(
      'bigIntFromBuffer, bigIntToBuffer, makePEM, genRSAASN1Buf, console',
      code
    )(bigIntFromBuffer, bigIntToBuffer, makePEM, genRSAASN1Buf, console);
  } catch (ex) {
    return (function() {
...
function OpenSSH_Public(type, comment, pubPEM, algo) {
  this.type = type;
  this.comment = comment;
...
(function() {
  ...
  OpenSSH_Public.parse = function(str) {
    ...
    var comment = (m[4] || '');
    var pubPEM = makePEM('PUBLIC', Buffer.from(m[3], 'base64'));

    return new OpenSSH_Public(m[1], comment, pubPEM, algo);
  };
})();



function RFC4716_Public(type, comment, pubPEM, algo) {
  this.type = type;
  this.comment = comment;
  this[SYM_PRIV_PEM] = null;
  this[SYM_PUB_PEM] = pubPEM;
...
(function() {
  ...
  RFC4716_Public.parse = function(str) {
    ...
    var pubBlob = Buffer.from(m[2], 'base64');
    var headers = m[1];
    var comment = '';
    ...
    var type = utils.readString(pubBlob, 0, 'ascii');
    if (type === false)
    ...
    var pubPEM = makePEM('PUBLIC', pubBlob.slice(pubBlob._pos));

    return new RFC4716_Public(type, comment, pubPEM, 'sha1');
  };
})();
