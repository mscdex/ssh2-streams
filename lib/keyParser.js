...
module.exports = function(data) {
  ...
  if ((m = RE_HEADER_OPENSSH_PRIV.exec(data[0]))
    ...
    if (!RE_HEADER_OPENSSH.test(data[1])) {
      ...
      } else if (privData.length > 4) {
        // ECDSA
        var fulltypeLen = privData.readUInt32BE(0, true);
        if (privData.length > (4 + fulltypeLen))
          ret.fulltype = privData.toString('ascii', 4, 4 + fulltypeLen);
      }
